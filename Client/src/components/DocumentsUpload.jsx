...
export default function DocumentsUpload({ documents, setDocuments, errors, setErrors }) {
  ...
  return <section className="registration-card"><div className="section-kicker">SECTION 3</div><h2>Documents</h2><p className="upload-note">Upload a recent candidate photo and DOB certificate. JPG, PNG and PDF are accepted (maximum 10 MB each).</p><div className="document-grid">{files.map(([field, label, accept]) => <div className={`document-upload ${errors[field] ? "input-error" : ""}`} key={field}><strong>{label}</strong><span>{documents[`${field}Name`] || "No file selected"}</span><label className="upload-btn" htmlFor={field}>{documents[`${field}Uploading`] ? "Uploading..." : documents[field] ? "Replace file" : "Choose file"}</label><input id={field} type="file" accept={accept} hidden onChange={(event) => upload(field, event.target.files?.[0])} />{errors[field] && <small className="error-text">{errors[field]}</small>}</div>)}</div></section>;
}
