...
export default function ParticipantForm({ formData, handleChange, errors }) {
  ...
  return <section className="registration-card">
    ...
    {formData.category && <div className="race-section"><div className="section-kicker">SECTION 2</div><h3>Race Selection — {formData.category}</h3><p>Select the race{multiRace ? "(s)" : ""} for this category only.</p><fieldset className={`race-options ${errors.races ? "input-error" : ""}`}><legend>{multiRace ? "Select Race(s) *" : "Select Race *"}</legend>{raceChoices.map((race) => <label key={race}><input type={multiRace ? "checkbox" : "radio"} name="races" value={race} checked={formData.races.includes(race)} onChange={handleChange} /> {race}</label>)}</fieldset>{errors.races && <small className="error-text">{errors.races}</small>}</div>}
  </section>;
}
