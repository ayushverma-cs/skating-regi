...
const downloadReceipt = (registration, photoUrl) => {
  const rows = [
    ...
    ["Registration Date", formatRegistrationDate(registration.createdAt)],
  ...
  const bannerUrl = new URL(smallBanner, window.location.origin).href;
  const documentHtml = `<!doctype html><html lang="en"><head><meta charset="utf-8"><title>Registration Receipt</title><style>body{margin:0;padding:32px;background:#f1f5f9;font-family:Arial,sans-serif;color:#0f172a}.receipt{max-width:760px;margin:auto;background:#fff;border:1px solid #cbd5e1;border-radius:16px;overflow:hidden}.banner{display:block;width:100%;height:auto}.header{position:relative;padding:28px 150px 28px 28px;background:#0f172a;color:#fff}.header p{margin:0 0 8px;color:#facc15;font-size:12px;font-weight:bold;letter-spacing:1px}.header h1{margin:0;font-size:28px}.photo{position:absolute;right:28px;top:20px;width:78px;height:94px;object-fit:cover;border:3px solid #facc15;border-radius:8px}.content{padding:28px}.id{padding:14px;border:1px dashed #d4a70b;border-radius:9px;background:#fffbeb;font-weight:bold}.id span{display:block;color:#64748b;font-size:11px;margin-bottom:4px}h2{font-size:16px;margin:26px 0 10px}.grid{display:grid;grid-template-columns:1fr 1fr;border-top:1px solid #e2e8f0;border-left:1px solid #e2e8f0}.field{padding:13px;border-right:1px solid #e2e8f0;border-bottom:1px solid #e2e8f0}.field span{display:block;color:#64748b;font-size:11px;font-weight:bold;letter-spacing:.5px}.field strong{display:block;margin-top:6px;font-size:14px}@media print{body{padding:0;background:#fff}.receipt{border:0}}</style></head><body><article class="receipt"><img class="banner" src="${escapeHtml(bannerUrl)}" alt="Championship banner"><header class="header"><p>ROLLER SPORT ASSOCIATION, MATHURA</p><h1>Registration Submitted</h1><div>1st Agra Regional Skating Championship – 2026</div>${photo}</header><main class="content"><div class="id"><span>REGISTRATION ID</span>${escapeHtml(registration.registrationId)}</div><h2>Registration Details</h2><section class="grid">${rows.map(([label, value]) => `<div class="field"><span>${escapeHtml(label)}</span><strong>${escapeHtml(value)}</strong></div>`).join("")}</section></main></article></body></html>`;
...
export default function RegistrationConfirmation() {
  ...
  return <main className="confirmation-page">
    <section className="confirmation-card" id="registration-slip">
      ...
      <div className="slip-grid payment-grid">{detail("Amount Submitted", `₹${registration.amountPaid || 500}`)}</div>
    </section>
